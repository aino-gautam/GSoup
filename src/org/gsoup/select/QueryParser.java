...
import org.gsoup.parser.TokenQueue;
...
class QueryParser {
    ...
    private void byAttribute() {
        ...
        if (cq.isEmpty()) {
            ...
            if (cq.matchChomp("="))
                ...
                evals.add(new Evaluator.AttributeWithValueMatching(key, RegExp.compile(cq.remainder())));
            else
    ...
    private static final RegExp NTH_AB = RegExp.compile("((\\+|-)?(\\d+)?)n(\\s*(\\+|-)?\\s*\\d+)?", "i");
    private static final RegExp NTH_B  = RegExp.compile("(\\+|-)?(\\d+)");

	private void cssNthChild(boolean backwards, boolean ofType) {
		String argS = tq.chompTo(")").trim().toLowerCase();
        MatchResult mAB = NTH_AB.exec(argS);
        MatchResult mB = NTH_B.exec(argS);
		final int a, b;
		if ("odd".equals(argS)) {
			a = 2;
			b = 1;
			...
			a = mAB.getGroup(3) != null ? Integer.parseInt(mAB.getGroup(1).replaceFirst("^\\+", "")) : 1;
			b = mAB.getGroup(4) != null ? Integer.parseInt(mAB.getGroup(4).replaceFirst("^\\+", "")) : 0;
		} else if (mB != null) {
			a = 0;
			b = Integer.parseInt(mB.getGroup(0).replaceFirst("^\\+", ""));
		} else {
    ...
    private void matches(boolean own) {
        ...
        if (own)
            evals.add(new Evaluator.MatchesOwn(RegExp.compile(regex)));
        else
            evals.add(new Evaluator.Matches(RegExp.compile(regex)));
    }
