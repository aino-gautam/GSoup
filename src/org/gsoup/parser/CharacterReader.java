...
class CharacterReader {
    ...
    CharacterReader(final String input) {
    ...
    char current() {
        return isEmpty() ? CharacterReader.EOF : input[pos];
    }

    char consume() {
        final char val = isEmpty() ? CharacterReader.EOF : input[pos];
        pos++;
        return val;
    }
    ...
    String consumeTo(final char c) {
        final int offset = nextIndexOf(c);
        if (offset != -1) {
            final String consumed = new String(input, pos, offset);
            pos += offset;
            return consumed;
        } else return consumeToEnd();
    }
    ...
    String consumeToEnd() {
        final String data = new String(input, pos, length-pos);
        pos = length;
        return data;
    }
...
//        String loScan = seq.toLowerCase(Locale.ENGLISH);
        final String loScan = seq.toLowerCase();
//        String hiScan = seq.toUpperCase(Locale.ENGLISH);
        final String hiScan = seq.toUpperCase();
